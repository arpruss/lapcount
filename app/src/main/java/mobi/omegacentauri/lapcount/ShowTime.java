...
abstract public class ShowTime extends Activity implements LifecycleOwner {
        ...
        switch (requestCode) {
            case PERMISSION_REQUEST_FINE_LOCATION: {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Log.d("lapcount", "fine location permission granted");
                } else {
                    final AlertDialog.Builder builder = new AlertDialog.Builder(this);
                    builder.setTitle("Functionality limited");
            ...
            case PERMISSION_REQUEST_BACKGROUND_LOCATION: {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Log.d("lapount", "background location permission granted");
                } else {
                    final AlertDialog.Builder builder = new AlertDialog.Builder(this);
                    builder.setTitle("Functionality limited");
...
// Detect the URL frame:
            ...
            public void didRangeBeaconsInRegion(Collection<Beacon> beacons, Region region) {
                if (beacons.size() > 0) {
                    ...
                    for (Beacon b : beacons) {
                        String id;
                        try {
                            id = UrlBeaconUrlCompressor.uncompress(b.getId1().toByteArray());
                        }
                        catch (Exception e) {
                            id = b.getId1().toString();
                        }
                        Log.i("lapcount", "Beacon "+id+" at "+b.getDistance()+" m RSSI "+b.getRssi()+ " "+b.getBluetoothName());
                    }
                }
            }
        });
        ...
        beaconManager.addMonitorNotifier(new MonitorNotifier() {
            ...
            public void didEnterRegion(Region region) {
                Log.i("lapcount", "I just saw an beacon for the first time!");
            }

            @Override
            public void didExitRegion(Region region) {
                Log.i("lapcount", "I no longer see an beacon");
            }

            @Override
            public void didDetermineStateForRegion(int state, Region region) {
                Log.i("lapcount", "I have just switched from seeing/not seeing beacons: "+state);
            }
        });

        beaconManager.startMonitoring(new Region("myMonitoringUniqueId2", null, null, null));
